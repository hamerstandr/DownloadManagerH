// Enhanced Firefox Extension for Download Manager Hamed
// Manifest V2 Background Script Implementation
...
browser.runtime.onInstalled.addListener(async (details) => {
    console.log('دانلود منجر حامد - افزونه Firefox نصب شد');
    ...
    if (details.reason === 'install') {
        showNotification(
            'دانلود منجر حامد',
            'افزونه Firefox با موفقیت نصب شد! اکنون می‌توانید لینک‌ها را به راحتی ارسال کنید.',
            'success'
        );
    }
});
...
async function saveSettings() {
    try {
        await browser.storage.sync.set({ extensionSettings });
    } catch (error) {
...
async function setupContextMenus() {
    try {
        ...
        console.log('منوهای زمینه‌ای پیشرفته Firefox با موفقیت ایجاد شدند');
    } catch (error) {
...
async function handlePageLinkDetection(tab) {
    try {
        ...
        const results = await browser.tabs.executeScript(tab.id, {
            code: `(${detectDownloadableLinks.toString()})()`
        });
        ...
        try {
            const response = await fetch(`${CONFIG.API_BASE_URL}${CONFIG.API_ENDPOINTS.ADD}`, {
                ...
                headers: {
                    ...
                    'User-Agent': 'DownloadManagerHamed-Firefox-Extension/2.0'
                },
...
// Handle extension icon click (Firefox uses browserAction instead of action)
...
async function handleAllMediaDownload(tab) {
    try {
        const results = await browser.tabs.executeScript(tab.id, {
            code: `(${detectAllMediaOnPage.toString()})()`
        });
        
        if (results && results[0]) {
            const mediaItems = results[0];
...
async function showExtensionHelp() {
    const helpContent = `
🚀 راهنمای استفاده از افزونه دانلود منجر حامد (Firefox)
...
در صورت بروز مشکل، با تیم پشتیبانی تماس بگیرید.
    ...
    browser.tabs.create({
        url: `data:text/html;charset=utf-8,${encodeURIComponent(`
            ...
            <head>
                <meta charset="UTF-8">
                <title>راهنمای افزونه دانلود منجر حامد (Firefox)</title>
                <style>
                    body { 
            ...
            <body>
                <div class="container">
                    <h1>🚀 راهنمای افزونه دانلود منجر حامد (Firefox)</h1>
                    <pre>${helpContent}</pre>
                </div>
            </body>
            </html>
...
console.log('دانلود منجر حامد - Firefox Background Script آماده است');