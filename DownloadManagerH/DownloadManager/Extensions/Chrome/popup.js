...
async function sendUrl() {
    ...
    try {
        ...
        const response = await fetch('http://127.0.0.1:24680/add/', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            throw new Error(errorText || 'خطا در ارسال لینک');
        }
    } catch (error) {
...
async function openMainApp() {
    try {
        // Try to focus the main application window
        const response = await fetch('http://127.0.0.1:24680/focus/', {
            method: 'POST'
        });
